...
| Job - External/Platform-Originated | A job created via integration with external service platforms or marketplaces rather than manual entry. | Enables businesses to track financials for work obtained through third-party platforms while maintaining consolidated reporting. |
| Job - One-Time | A job that occurs once without an expected repetition pattern. | The standard job type for ad-hoc services or projects with clear start and end dates. |
| Job - On-Demand/Condition-Triggered | A job that is performed when specific external conditions are met rather than on a fixed schedule. | Common for weather-dependent services (e.g., snow removal after snowfall) where timing is unpredictable but triggering conditions are defined. |
| Job - Recurring | A job that repeats on a defined schedule for the same client. | Creates efficiency through predictable timing and consistent service delivery (e.g., weekly lawn mowing every Tuesday). |
...
| Job | Specific service instance performed for a client. | Job Date, Service Type, Status (Planned/In-Progress/Complete), Description, Location, Season (Summer/Winter/Other), JobType (One-Time/Recurring/On-Demand), RecurrencePattern (null for One-Time jobs), ParentJobId (for recurring instances), TriggerConditions (for on-demand jobs) | Belongs to Client, Has Service Type, Has Expense Allocations, Has Income, Involves Contractors, Can be duplicated from existing Job |
